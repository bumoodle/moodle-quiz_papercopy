...
include_once($CFG->dirroot .'/lib/zipstream.php');
...
class printable_copy_helper
{
    ...
    public function print_batch($batch_id, $key = false, $key_only = false, $to_zip = false)
    {
        ...
        $batch_info = $DB->get_record('paper_batches', array('id' => $batch_id));
    ...
    public function prerender_batch($batch_id, $force_rerender, $progress_callback)
    {
        ...
        //load the batch information from the database
        $batch_info = (array)$DB->get_record('paper_batches', array('id' => $batch_id));
        ...
        //$DB->update_record('paper_batches', $batch_info);
    }
    ...
    public function zip_question_usage_by_activities($quba_array, $key = false, $key_only = false, $output = true, $prefix='quiz', $cache_id = 0, $progress_callback = null, $save = true)
    {
        ...
        if($output)
        {
            ...
            if($save)
            {
                ...
                $opt['save_to'] = $target_zip = tempnam(sys_get_temp_dir(), 'moodlepdf'); 
        ...
        /*
        //if $output is true, output the zip file, and delete it
        if($output)
        {
        ...
        */
        ...
        if(!$output || $save)
        {
            ...
            //copy the created zip-file into the Moodle data store
